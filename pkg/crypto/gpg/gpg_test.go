...
// minimal mock HTTP client used by GPG.FetchGPGPubKeyFromKeyServer.
...
func TestReadPublicPrivateKeyFromFile(t *testing.T) {
	g := &GPG{}

	pubPath := writeTempFile(t, "pub", "pubdata")
	defer func() {
		_ = os.Remove(pubPath)
	}()
	privPath := writeTempFile(t, "priv", "privdata")
	defer func() {
		_ = os.Remove(privPath)
	}()

	g.PublicKeyPath = pubPath
	g.PrivateKeyPath = privPath

	pub, err := g.ReadPublicKeyFromFile()
	require.NoError(t, err)
	require.Equal(t, "pubdata", pub)

	priv, err := g.ReadPrivateKeyFromFile()
	require.NoError(t, err)
	require.Equal(t, "privdata", priv)
}

func TestFetchGPGPubKeyFromKeyServer_Errors(t *testing.T) {
	g := &GPG{}

	// empty keyID.
	_, err := g.FetchGPGPubKeyFromKeyServer("", "http://example.com")
	require.ErrorIs(t, err, ErrKeyIDEmpty)

	// empty keyServerURL.
	_, err = g.FetchGPGPubKeyFromKeyServer("ABC", "")
	require.ErrorIs(t, err, ErrKeyServerURLEmpty)

	// HTTP client error.
	mock := &mockHTTPClient{resp: nil, err: errors.New("network")}
	g.httpClient = mock
	_, err = g.FetchGPGPubKeyFromKeyServer("ABC", "http://ks")
	require.Error(t, err)
}

func TestFetchGPGPubKeyFromKeyServer_SuccessAndNonOK(t *testing.T) {
	g := &GPG{}
	...
	// success path: body content written to temp file and PublicKeyPath set.
	...
	pathPtr, err := g.FetchGPGPubKeyFromKeyServer("ABC", "http://ks")
	require.NoError(t, err)
...
func TestEncryptFile_Errors(t *testing.T) {
	g := &GPG{}

	// empty input path.
	_, err := g.EncryptFile("")
	require.ErrorIs(t, err, ErrEmptyInputFilePath)

	// invalid public key content -> ReadArmoredKeyRing should fail.
	inputPath := writeTempFile(t, "in", "plaintext")
	defer func() {
		_ = os.Remove(inputPath)
	}()

	pubPath := writeTempFile(t, "pubinvalid", "not a key")
	defer func() {
		_ = os.Remove(pubPath)
	}()

	g.PublicKeyPath = pubPath
	_, err = g.EncryptFile(inputPath)
	require.Error(t, err) // expect openpgp.ReadArmoredKeyRing or related failure
}
...
func TestDecryptFile_Errors(t *testing.T) {
	...
	// empty input path.
	_, err := g.DecryptFile("", "pass")
	require.ErrorIs(t, err, ErrEmptyInputFilePath)

	// invalid private key content -> ReadArmoredKeyRing should fail.
	encPath := writeTempFile(t, "enc", "encrypteddata")
	defer func() {
		_ = os.Remove(encPath)
	}()

	privPath := writeTempFile(t, "privinvalid", "not a key")
	defer func() {
		_ = os.Remove(privPath)
	}()

	g.PrivateKeyPath = privPath
	_, err = g.DecryptFile(encPath, "pass")
	require.Error(t, err)
}
