...
const (
	...
	testFileContet = "This is a sample text file created in the temp directory."
)
...
func setupTestFile() (string, error) {
	...
	_, err = tempFile.WriteString(testFileContet)
	if err != nil {
		return "", err
	}
...
func TestGPGDecrypt(t *testing.T) {
	...
	assert.NoError(t, err)
	assert.Equal(t, testFileContet, string(decryptedData))
